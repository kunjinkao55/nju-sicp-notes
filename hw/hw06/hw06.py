...
class shiftcipher(methods):
    ...
    def encrypt(self, message):
        ...
        temp=[]
        for i in message:
            temp.append(chr(97+(7+ord(i)+self.shift)%26))
        return "".join(temp)
...
class multipleencryption(extensions):
    ...
    def decorator(self, function, message):
        ...
        "*** YOUR CODE HERE ***"
        #func is an instance of methods
        if self.counts == 0:
            return message
        a=function(message)
        tempcount=self.counts
        while(tempcount>1):
            a=function(a)
            tempcount-=1
        return a
...
class splitencryption(extensions):
    ...
    def decorator(self, function, message):
        ...
        temp2=''
        for i in message:
            if(count%self.x==0 and count!=0):
                temp1+=i
            else:
                temp2+=i
            count+=1
        return function(temp2+temp1)
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        copy_deck=opponent.deck.copy()
        new_deck=[]
        new_new_deck=[]
        while(not copy_deck.is_empty()):
            new_deck.append(copy_deck.draw())
        for i in new_deck:
            if not (i.attack==other_card.attack and i.defense==other_card.defense):
                new_new_deck.append(i)
            opponent.deck=Deck([i.copy() for i in new_new_deck])

        old_deck=[]
        ano_copy_deck=player.deck.copy()
        while(not ano_copy_deck.is_empty()):
            old_deck.append(ano_copy_deck.draw())
        for i in old_deck:
            i.attack+=other_card.attack
            i.defense+=other_card.defense
        player.deck=Deck([i.copy() for i in old_deck])
