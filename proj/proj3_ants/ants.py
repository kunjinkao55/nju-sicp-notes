...
class Place:#创建place的时候会紧接着创建它后面那个？
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN PROBLEM 8
            assert  issubclass(type(self),ContainerAnt)!=issubclass(type(place.ant),ContainerAnt) and (issubclass(type(self),ContainerAnt) or (issubclass(type(place.ant),ContainerAnt) and place.ant.ant_contained == None )), 'Two ants in {0}'.format(place)
            if not issubclass(type(self),ContainerAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        leng=0
        while((not now_place.is_hive)):
            if now_place.bees and self.min_range<=leng<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN PROBLEM 5
        tplace=self.place
        if tplace and tplace.bees:
            i = len(tplace.bees[::])
            while i > 0:
                if self.health-amount>0:
                    tplace.bees[i-1].reduce_health( amount)
                else:
                    tplace.bees[i-1].reduce_health( amount+self.damage)
                i-=1
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN PROBLEM 8
        if type(other)!=ContainerAnt and self.ant_contained==None:
            return True
        return False
        # END PROBLEM 8
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate): 
        if self.place and self.place.bees:
            ...
            while i > 0:
                self.place.bees[i-1].reduce_health(self.damage)
                i-=1
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_waterproof==False:
            ...
            #Insect.reduce_health(insect,insect.health)也可以
        # END PROBLEM 10
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN PROBLEM 12
        tplace=self.place.exit
        while(tplace):
            if tplace.ant:
                tplace.ant.buff()
                if isinstance(tplace.ant,ContainerAnt) and tplace.ant.ant_contained:
                    tplace.ant.ant_contained.buff()
            tplace=tplace.exit
