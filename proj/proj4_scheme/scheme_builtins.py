...
def strict_scheme_apply(fn, args, env):
    ...
    if isinstance(res, Unevaluated):
        return scheme_eval(res.expr, res.env,True)
    else:
        return res
...
def scheme_equalp(x, y):
    if scheme_pairp(x) and scheme_pairp(y):
        return scheme_equalp(x.first, y.first) and scheme_equalp(x.rest, y.rest)#rest这里实现了递归比较
...
def scheme_cdr_stream(x):
    ...
    return scheme_force(x.rest)#将promise eval 为表达式
