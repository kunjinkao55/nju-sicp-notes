...
def compose(h, g):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda x: h(g(x))#传入参数x，返回的是一个函数
...
def accumulate(combiner, base, n, f):
    ...
    if n==0:
       return base
    result=combiner(f(1),base)
    for i in range(2,n+1):
     result =combiner(f(i),result)
    return result
...
def missions(f):
    ...
    def mission3(x, y):
        def mission3_inner(x):
            if x == y:
                return mission3(x, y + 1)

        if y == 5:
            print('MISSION 3 SOLVED')
            return 'The cake is a lie.'
        else:
            return mission3_inner#(x)
...
def get_the_cake(missions):
    ...
    ['Expr', 'Return']
    """
    return missions(lambda a: a*2)(lambda a: a)(0)(1)(2)(3)(4)


def protected_secret(password, secret, num_attempts):
    """
    Returns a function which takes in a password and print the SECRET if the password entered matches
...
three = successor(two)#=lambda x:f(f(f(x)))
...
def mul_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    temp1=add_church(m,m)
    num=church_to_int(n)-1
    while num>1:
        temp1=add_church(temp1,m)
        num=num-1
    return temp1
