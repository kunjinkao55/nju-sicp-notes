...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        temp_operator = self.operator.eval(env)
        temp_operands=[]
        for oper in self.operands:
            temp_operands.append(oper.eval(env))
        return temp_operator.apply(temp_operands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):#arguments为实参列表
        """
        >>> from reader import read
        ...
        #     temp_parent[self.parameters[i]]=arguments[i]#使用当前环境的变量替换父环境的变量
        for para, arg in zip(self.parameters, arguments):
            temp_parent[para] = arg#使用当前环境的变量替换父环境的变量另外的办法：当前环境的变量名和实参能够匹配上的，替换掉父环境对应变量名的值
        return self.body.eval(temp_parent)   
